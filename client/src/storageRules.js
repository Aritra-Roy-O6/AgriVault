export const storageRequirements = {
  clothes: { env: ["dry"], secure: false, covered: true },
  fabrics: { env: ["dry"], secure: false, covered: true },
  grains: { env: ["dry"], secure: false, covered: true },
  vegetables: { env: ["cool"], secure: false, covered: true },
  fruits: { env: ["cool"], secure: false, covered: true },
  electronics: { env: ["dry"], secure: true, covered: true },
  cosmetics: { env: ["dry"], secure: true, covered: true },
  furniture: { env: ["covered"], secure: false, covered: true },
  decorations: { env: ["dry"], secure: false, covered: true },
  tiles: { env: ["covered"], secure: false, covered: true },
  "steel/iron": { env: ["covered"], secure: false, covered: true },
  produce: { env: ["dry", "cool"], secure: false, covered: true },
};

export const defaultCategories = Object.keys(storageRequirements);
...
export function scoreWarehouseMatch(warehouse, requestedCategory, requestedLocation = "") {
  ...
  const environmentTags = (warehouse.environmentTags || []).map(normalize);
  ...
  if (requirements?.env?.some((tag) => environmentTags.includes(normalize(tag)))) {
    score += 20;
